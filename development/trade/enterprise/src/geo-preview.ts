...
export function productPreview(record: GeoRecord, now: Date) {
  ...
  const offers = product.offers.flatMap(offer => {
    const price = known.find(claim => offer.claimIds.includes(claim.id) && claim.property === 'price')
    if (!price || price.value.type !== 'number' || price.value.value < 0) return []
    return [{ '@type': 'Offer', '@id': `${product.identity.canonicalUrl}#offer-${encodeURIComponent(offer.id)}`, seller: { '@type': 'Organization', '@id': offer.seller.url, name: offer.seller.name }, priceSpecification: { '@type': 'UnitPriceSpecification', price: price.value.value, priceCurrency: offer.currency, unitText: offer.unit }, eligibleRegion: offer.regions, validThrough: offer.validUntil }]
  })
  if (offers.length) jsonLd.offers = offers
  const details = product.offers.map(offer => `<section><h3>${escape(offer.id)}</h3><p>${reference(offer.seller)} · ${escape(offer.currency)} / ${escape(offer.unit)} · ${escape(offer.regions.join(', '))} · ${escape(offer.validUntil)}</p>${facts(claims.filter(claim => offer.claimIds.includes(claim.id)))}</section>`).join('')
