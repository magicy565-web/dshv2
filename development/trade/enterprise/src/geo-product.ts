...
export type GeoLayer = 'entity' | 'understanding' | 'facts' | 'evidence' | 'discovery'
...
export function productReadiness(product: GeoProduct | undefined, confirmed: boolean, now: Date) {
  ...
  const layers = Object.fromEntries((['entity', 'understanding', 'facts', 'evidence', 'discovery'] as const).map(layer => [layer, issues.some(issue => issue.layer === layer) ? 'PARTIAL' : 'READY']))
  return { layers, issues, previewReady: !issues.some(issue => issue.layer !== 'discovery'), publicationStatus: 'not_published' as const }
}
