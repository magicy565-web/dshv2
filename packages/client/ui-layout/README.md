...
`selectPanel(id)` checks the live `main` registry before changing selection; an absent key throws and leaves the current panel intact. `registerPanelGuard(allow)` installs a synchronous check for valid panel selections in registration order. Returning false keeps selection and pending navigation intact. The caller releases the returned disposer on unmount; layout disposal clears all guards. Guards cover panel selection, not Session creation or plugin removal. `beginNavigation()` returns an abort signal for an asynchronous UI navigation. A later call, an allowed panel selection (including repeated selection), or layout disposal aborts that signal without cancelling underlying Session creation. Consumers check the signal before committing navigation or moving drafts.
