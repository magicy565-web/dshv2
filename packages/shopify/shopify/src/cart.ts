...
export interface CartLine {
  ...
  readonly cost: {
    readonly amountPerQuantity: string
...
export class StorefrontCartClient {
  ...
  async create(lines: readonly CartLineInput[], buyerIdentity?: Readonly<Record<string, unknown>>): Promise<CartSnapshot> {
    const data = await this.client.execute<{
      cartCreate: { cart: CartPayload['cart']; userErrors: Array<{ message: string }> }
    }>(
  ...
  async remove(cartId: string, lineIds: readonly string[]): Promise<CartSnapshot> {
    const data = await this.client.execute<{
      cartLinesRemove: { cart: CartPayload['cart']; userErrors: Array<{ message: string }> }
    }>(
  ...
  private async mutate(operation: 'cartLinesAdd' | 'cartLinesUpdate', cartId: string, lines: readonly unknown[]): Promise<CartSnapshot> {
    const data = await this.client.execute<Record<string, {
      cart: CartPayload['cart']
      userErrors: Array<{ message: string }>
    }>>(
      `mutation CartLines($cartId: ID!, $lines: [CartLineInput!]!) { ${operation}(cartId: $cartId, lines: $lines) { cart { ${CART_FIELDS} } userErrors { message } } }`,
      { cartId, lines },
    )
    const payload = data[operation]
    if (payload === undefined) throw new Error(`Shopify response did not contain ${operation}`)
  ...
  private unwrap(payload: { cart: CartPayload['cart']; userErrors: Array<{ message: string }> }): CartSnapshot {
    ...
    return {
      id: cart.id, checkoutUrl: cart.checkoutUrl, updatedAt: cart.updatedAt,
      lines: cart.lines.nodes, subtotalAmount: cart.cost.subtotalAmount.amount,
      totalAmount: cart.cost.totalAmount.amount,
      currencyCode: cart.cost.totalAmount.currencyCode,
      ...(cart.cost.totalTaxAmount ? { totalTaxAmount: cart.cost.totalTaxAmount.amount } : {}),
    }
  }
}
