...
选项要求显式指定 `themeId`。工作流将版本与存储状态核对，解析租户所属连接，并拒绝不存在或已经在线的所选主题。只有分类为 transient 或 rate-limit 的 `ShopifyApiError` 会重试。权限错误和未知错误立即停止。审计回调失败向调用方传播，不会重复已成功的提供方发布。尝试状态随 Site 任务持久保存。详细审计回调记录保存在当前进程，返回独立记录。

## Known Limitations and Deferred Work

- 受控渲染器生成 Shopify 主题模板，拒绝任意源码项目。真实发布要求已授权提供方和已审核内容。
- 企业工作器负责持久化目标审核和自动队列执行。分布式锁与自动回滚由部署侧负责。
