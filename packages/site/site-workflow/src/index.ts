...
export class SitePublishWorkflow {
      ...
      for (let number = 1; number <= maxAttempts; number += 1) {
        ...
        try {
          ...
          await provider.publish(storeSpec, {
            connectionId: site.connectionId, themeId: theme.id, files: this.render(resolved), idempotencyKey: key,
          })
        } catch (error) {
          const failed = {
  ...
  private render(revision: SiteRevision): Readonly<Record<string, string>> {
    ...
    const files: Record<string, string> = {
      'site.revision.json': JSON.stringify(revision.changeSet),
      'sitemap.xml': renderSitemap(revision, origin),
      'robots.txt': renderRobots(origin),
    }
