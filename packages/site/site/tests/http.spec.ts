...
interface SiteJson { readonly id: string; readonly tenantId: string; readonly connectionId?: string }
interface RevisionJson { readonly id: string; readonly source?: string }
interface DiffJson { readonly changed: readonly { readonly after: { readonly title: string } }[] }
...
describe('site editor HTTP', () => {
  it('creates and restores a site without commerce while still authorizing supplied connections', async () => {
    ...
    const site = await readJson<SiteJson>(response)
  ...
  it('resolves routes and methods before parsing action parameters or bodies', async () => {
    ...
    const site = await readJson<SiteJson>(await request('/sites', { name: 'Store', connectionId: 'store' }))
  ...
  it('returns a missing-page response and previews inherited pages', async () => {
    ...
    const first = await readJson<RevisionJson>(await request(`${path}/revisions`, { changeSet: {
      pages: [{ id: 'home', kind: 'home', path: '/', title: 'Home' }],
    } }))
    const inherited = await readJson<RevisionJson>(await request(`${path}/revisions`, { changeSet: { baseRevisionId: first.id } }))
    ...
    expect(await readJson<{ readonly error: string }>(missing)).toEqual({ error: 'page not found' })
    expect((await request(`${path}/preview?revisionId=${inherited.id}`)).status).toBe(400)
    const cleared = await readJson<RevisionJson>(await request(`${path}/revisions`, { changeSet: { baseRevisionId: inherited.id, pages: [] } }))
  ...
  it('queues publication for an existing revision and rejects stale expectations', async () => {
    ...
    const site = await readJson<SiteJson>(await request('/sites', { name: 'Store', connectionId: 'store' }))
    const path = `/sites/${site.id}`
    const revision = await readJson<RevisionJson>(await request(`${path}/revisions`, { changeSet: {} }))
    const queued = await request(`${path}/publish`, { revisionId: revision.id, expectedRevisionId: revision.id })
    expect(queued.status).toBe(202)
    expect((await readJson<{ readonly revisionId: string }>(queued)).revisionId).toBe(revision.id)
  ...
  it('creates, edits, compares and previews a site while enforcing the observed revision', async () => {
    ...
    const site = await readJson<SiteJson>(created)
    ...
    const first = await readJson<RevisionJson>(firstResponse)
    ...
    const second = await readJson<RevisionJson>(secondResponse)
    const diff = await readJson<DiffJson>(await request(`${path}/diff?revisionId=${second.id}&baseRevisionId=${first.id}`))
    const change = diff.changed[0]
    if (change === undefined) throw new Error('diff did not contain the updated page')
    expect(change.after.title).toBe('Updated')
    ...
    expect((await readJson<RevisionJson>(rolled)).source).toBe('rollback')
  })
