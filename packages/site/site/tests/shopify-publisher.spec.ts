...
describe('createShopifySitePublisher', () => {
  it('renders and publishes one revision with a stable idempotency key', async () => {
    ...
    const publisher = createShopifySitePublisher({ publish } as never, ShopifyThemeId('gid://shopify/Theme/9'), (_site, revision) => ({ 'templates/index.html': revision.id }))
    ...
    expect(publish).toHaveBeenCalledWith(expect.objectContaining({ mode: 'oauth' }), expect.objectContaining({ themeId: 'gid://shopify/Theme/9', idempotencyKey: 'site-revision:revision', files: { 'templates/index.html': 'revision' } }))
  })
