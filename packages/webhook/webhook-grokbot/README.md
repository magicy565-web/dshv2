...
- name: '@deepseek-ai/dsh-webhook-grokbot'
  config:
    ...
    leaseSeconds: 120
    wakeUrl: https://api.example.com/automations/webhook/<routine-id>
    senderKeyEnv: GROKBOT_SENDER_KEY
```
...
When `wakeUrl` and `senderKeyEnv` are set, task creation sends a best-effort webhook wake-up with the sender key; task delivery remains safe because the Routine must still claim the task.
