...
export interface Config { readonly path: string; readonly authEnv: string; readonly maxBodyBytes: number; readonly leaseSeconds: number; readonly wakeUrl?: string; readonly senderKeyEnv?: string }
export const Config: s<Config> = s.object({ path: s.string().required(), authEnv: s.string().role('credential-ref').required(), maxBodyBytes: s.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER).required(), leaseSeconds: s.number().step(1).min(5).max(3600).default(120), wakeUrl: s.string().role('url').default(''), senderKeyEnv: s.string().role('credential-ref').default('') })
...
export class GrokbotTaskService extends Service {
  ...
  private readonly senderKeyEnv: string
  constructor(ctx: Context, config: Config) {
    super(ctx, 'grokbotTasks'); this.readyDomain = ctx.storageDomain.open(taskDomain); this.wakeUrl = config.wakeUrl ?? ''; this.senderKeyEnv = config.senderKeyEnv ?? ''
