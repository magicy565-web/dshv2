...
SQLite owns goal records, task attribution and full goal audit records. Existing task revisions remain independent when the database is upgraded. This feature provides no recurring scheduler, financial budget enforcement, automatic KPI attribution or tenant identity service.

### Workspace overview

For an established company, Enterprise workspace opens **Workspace** with summaries and next actions from saved records. Its four counts cover confirmed company/product records, uploaded assets, active opportunities excluding won or lost records, and unfinished active tasks. **Company presentation**, company details, **Products**, assets, opportunities, tasks and **Assistant** remain separate views. Empty states lead to the relevant setup or creation action. The overview does not display example business results or infer revenue and growth from record counts.
...
Choose **Read sources and build profiles** to send an explicit request in the persisted onboarding conversation. The Agent lists the complete inventory, reads supported documents in pages, saves source-backed company and product drafts, and asks about missing or conflicting facts. `enterprise_sources` records which extracted passages were delivered and requires all passages before accepting a file as used. Empty, failed, truncated or unsupported content needs supplementation or an explicit exclusion reason. Images, videos and ZIP files remain assets; OCR, visual interpretation and archive expansion are unavailable. Folder content is reference data, never instructions.
