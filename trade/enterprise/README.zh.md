...
可按需使用的 **FORM** 组件集提供 13 个 React 组件，包含作用域内的松绿、暖白与青柠色变量、深浅主题、响应式布局、键盘焦点和减少动态效果支持。[组件](src/design/components.tsx)接收调用方提供的文案、数据与回调；[样式](src/design/design.css)作用于 `.td-root` 内。组件集不替换正在使用的企业主题，也不发起业务请求。[交互预览](src/design/showcase.tsx)使用虚构记录，筛选、核对、产品收藏和草稿状态仅保留在内存中。

| 组件 | 用途 |
| --- | --- |
| `ActionButton`、`StatusBadge`、`TextField`、`SegmentedControl` | 操作、具名状态、无障碍输入与受控筛选。 |
| `Surface`、`SectionHeading`、`MetricCard`、`ProgressSteps` | 布局、标题、外部提供的指标和明确的流程进度。 |
| `SourceRow`、`ReviewCard`、`ProductCard` | 文件元数据、有来源的审阅与产品展示。 |
| `EmptyState`、`ActivityList` | 下一步操作与调用方提供的活动记录。 |

从 `src/design/components.tsx` 导入组件，在调用方浏览器构建中引入一次 `src/design/design.css`。用 `<div className="td-root" data-theme="light">` 包裹组件组合；使用 `dark` 切换主题，或在该根元素上覆盖 `--td-*` CSS 变量。组件通过属性接收本地化标签。审阅控件仅调用回调：Host 仍负责版本检查、权限和持久确认。`showcase.css` 仅用于预览，不应引入业务页面。

在仓库根目录执行 `node trade/enterprise/build-design.mjs`，再用浏览器打开 `.trade-runtime/design-system/index.html`。该独立文件不需要服务、网络、凭据或真实企业数据。`node node_modules/typescript/bin/tsc -p trade/enterprise/tsconfig.design.json` 检查组件集；`node --test trade/enterprise/test/design-browser.test.mjs` 检查筛选、核对状态、弹窗焦点恢复、邮箱错误、主题、双语与窄屏溢出。截图保存在 `.trade-runtime/design-evidence-*`。
...
**业务目标**保存目标、成功标准、负责人标签、期限和实际结果。用户凭已读取版本编辑、暂停、标记达成、归档或恢复目标。达成必须填写结果，且始终由人确认；任务数量不证明营收、增长或目标完成。这些企业记录独立于 Harness 会话目标。

企业任务可以关联推进中且未归档的目标，也可以保持独立。关联目标的任务完成时必须记录事实与依据。暂停或归档目标保留已有任务关联和结果，但禁止新关联。任务编辑保留版本历史，并使绑定旧任务版本的审批失效。电脑工作经人工验收后，其提交结果写入关联企业任务；现有人工验收和文件检查继续生效。

**用 AI 梳理下一步**打开新会话，通过只读 `enterprise_work` 工具读取已保存目标和任务。工具按 `maxWorkItems` 分页，默认每页 20 条，返回的上下文通过普通工具结果写入会话日志。它不编辑任务、不确认目标达成，也不授权对外操作。[业务目标决策](../../.agents/notes/implemented/feature/2026-09-21-enterprise-business-goals.zh.md)说明范围及取舍。

SQLite 保存目标、任务关联和完整目标审计记录。数据库升级后，已有任务版本保持独立。新建目标和任务不依赖浏览器仅在安全上下文提供的 UUID API。保存失败时保留编辑草稿及其已读取版本；发生版本冲突后，需要刷新记录并重新打开编辑框。这项功能不提供周期调度、资金预算执行、自动 KPI 归因或租户身份服务。

### 工作台概览

已有企业档案时，企业空间默认打开**工作台**：当前业务目标及其事实上下文、由进行中任务和管道机会组成的工作流、需要用户处理的事项队列（受阻任务、待确认档案、文字提取失败、新线索），以及最近成果时间线。新工作空间以创建第一个目标的单一提示开始。**企业展示**、企业档案、**产品中心**、资料、机会、任务和 **Assistant** 保留为独立视图。概览全部派生自已保存的记录，不展示示例业务结果，也不根据记录数量推断营收和增长。这套呈现背后的共享工作台组件——使命面板、台账行、决策队列、成果时间线——集中在 `workbench-ui.tsx`；[Trade 工作台组件语言](../../.agents/notes/implemented/feature/2026-09-21-trade-workbench-component-language.zh.md)记录模块范围。
...
企业档案尚未建立时，以及 Assistant 和企业资料中，都可以**选择文件夹**。核对选中的相对路径后，点击**导入所选资料**。子文件夹保留原路径标记。隐藏文件、凭据目录和不支持的格式会跳过，不上传其字节。清单记录每个选中文件、上传结果、提取片段数量和 Agent 处理结论。后续文件失败不影响已成功保存的文件；刷新后重新选择原文件夹，可以续传等待或失败的项目。`maxSourceFiles` 限制每次导入的文件数量，默认 1,000。

导入失败后保留已选文件，可以在同一面板重试。重试这次选择会复用导入编号，仅传输等待或失败的文件；上传响应丢失时，根据已保存清单核实结果。汇总分别显示已保存文件、等待上传、上传失败、已跳过、已返回片段和需要用户补充的资料数量。空文件与已删除来源显示具体原因。

点击**读取资料并建档**，向持久化建档会话发送明确请求。Agent 遍历完整清单，分页读取支持的文档，保存有来源的企业与产品草稿，再询问缺失或冲突的信息。`enterprise_sources` 记录已返回的提取片段，只有全部片段已读取，文件才能标记为已使用。空文档、解析失败、截断或不支持的内容需要补充资料或明确排除理由。支持的图片和扫描 PDF 可通过下述识别操作提供文字；视频和 ZIP 保留为素材，不提供视觉理解和压缩包展开。文件夹内容只作为参考资料，不作为指令。

**图片与扫描 PDF 识别。** 在资料清单中，对 PNG、JPEG、WebP、AVIF 或 PDF 点击**识别图片 / 扫描件**。识别通过 Tesseract 和 PDF.js 在本机执行。首次使用会下载语言数据；也可用 `ocr.langPath` 指定已有语言数据目录或网址。原文件保持私有且不变。**核对识别文字**按原始页码展示片段；下载原件并逐项比对后再确认。如文字错误，请补充更正的文字文档或在建档对话中说明，不要确认错误内容。

OCR 片段追加到已有文档文字之后，保留原文字引用。识别结果包含页码引用、语言及持久化的人工核对回执。成功的识别结果不可修改，重复请求返回同一回执。Agent 可以读取待核对文字，但人工核对前不能将该来源标记为已使用。新增片段重置该文件的阅读和处理记录，并清除已完成建档标记。识别错误保留已有文字，允许重试；失败或超限不保存部分 OCR 结果。

Trade 配置启用可选的 `ocr`，默认值为 `language: eng+chi_sim`、`timeoutMs: 120000`、`maxPages: 30`、`maxPixels: 16000000` 和 `renderScale: 2`。`maxExtractedCharacters` 和 `knowledgeChunkCharacters` 同样限制 OCR 输出。每个企业 Host 同时运行一项识别；取消和超时会先终止工作线程，再释放操作。PDF 按页顺序渲染，包括文字与图片混排的页面。像素限制作用于输出画布，不是嵌入图片解码的硬内存配额。手写文字、复杂表格、阅读顺序及低清晰度扫描仍需人工检查；OCR 不等于视觉理解或独立事实核验。

聊天内的最终范围确认包括选中的档案和资料处理结论。等待上传、未处理资料以及已使用来源缺失都会阻止完成。确认期间资料发生变化时，过期回答不能提交。导入其他文件夹或删除来源会清除完成标记。确认保存内部企业档案，不代表认证或公开发布。

资料进度要求完整阅读或记录排除理由，仅上传不会完成该步骤。档案审阅涵盖当前企业和产品草稿；完成后则依据确切确认的范围。准确且唯一的索引引用会自动把文档关联到已保存草稿；仍可显式关联图片和视频。同名引用存在歧义时必须提供文件 ID。内部产品档案可以保存有来源的规格而不提供公开产品网址；面向发布的结构化事实属于选填内容，仍须满足自身要求。

**产品中心**展示已有 GEO 产品记录。用户可以新增草稿、编辑介绍和有来源的信息项、上传或关联素材、搜索、审阅、确认、归档及恢复产品。结构化声明和报价继续通过 Agent 完善。修改已确认产品时建立独立草稿；过期编辑会失败，不覆盖其他版本。归档作用于产品的整个修订链，保留文件和内容。已发布或已关联 Shopify 的产品需要先处理发布状态才能归档。[文件夹建档与产品归属](../../.agents/notes/implemented/feature/2026-09-21-enterprise-folder-onboarding.zh.md)记录这些决策。

首次进入尚未建立企业档案的项目时，**企业空间**内置的 **Assistant** 自动展示业务、来源、草稿审阅及范围确认四项进度，并准备名为**企业初始化 · 档案建档**的固定会话。点击**开始建档对话**或**读取资料并建档**会发送原生 `/product-geo` 调用，加载随插件提供的 [skill](skills/product-geo/SKILL.md)。没有资料时先询问企业名称、主营业务和服务对象，每次最多三个问题；有资料时读取已有答案。提问和档案章节随实际业务组织。已有档案的项目从企业空间内的 **Assistant** 按钮或标签进入建档，也可使用 AI 创作。侧栏只保留企业空间入口。
...
企业展示标签页提供可阅读的业务概览。卡片展示产品供给、解决方案、能力和案例；选择卡片后打开条件、关联内容和来源。**与 Agent 建档**通过对话更新档案。**管理与操作**包含采购操作及**逐项维护**，每次编辑一个对象或来源。未保存修改需要明确选择是否放弃。草稿提示及已确认版本切换标明当前显示的版本。供给层级涵盖品类、系列、产品和变体。适用买家、市场、合作模式和保密说明明确保存。证据卡可打开原文或已上传媒体，保留类别、可信度、摘录和发布时间。解决待确认问题，保存草稿，再审阅完整版本后确认。查询使用字面关键词，由调用 Agent 理解自然语言采购请求。扫描文档通过上述资料清单中的识别与核对流程提供文字。
...
预编写的[来源到草稿回放](../../snapshots/session/enterprise-source-draft/snapshot.yml)验证文档读取、自动文件关联和持久保存，不授予确认。验证真实模型文件夹流程时，将 `DSH_ENTERPRISE_LIVE_HOME` 指向包含模型设置和凭据引用的现有 Harness 主目录，再执行 `node --test trade/enterprise/test/folder-model.e2e.mjs`。测试在私有临时部署中导入虚构企业与两款产品资料，由测试用户回答审阅，检查最终范围及刷新后的持久化，并将证据写入 `.trade-runtime/folder-model-evidence-*`，不改动所选主目录的业务数据。

运行识别测试时，将 `DSH_ENTERPRISE_OCR_LANG_PATH` 指向包含 Tesseract `eng.traineddata.gz` 与 `chi_sim.traineddata.gz` 的本机目录。构建后运行 `node --test --test-name-pattern='OCR|folder' trade/enterprise/test/host.test.mjs` 和 `node --test trade/enterprise/test/folder-browser.test.mjs`。未设置该变量时跳过引擎与浏览器识别检查，取消、超时和资源限制检查仍然运行。预编写的 [OCR 草稿回放](../../snapshots/session/enterprise-ocr-draft/snapshot.yml)检查页码引用与待人工核对状态能否保留在模型可见的文档结果中。这些固定资料不衡量任意企业文档的识别准确性。
