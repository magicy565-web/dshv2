...
SQLite 保存目标、任务关联和完整目标审计记录。数据库升级后，已有任务版本保持独立。这项功能不提供周期调度、资金预算执行、自动 KPI 归因或租户身份服务。

### 工作台概览

已有企业档案时，企业空间默认打开**工作台**，根据已保存记录显示摘要和下一步操作。四项统计涵盖已确认企业及产品记录、已上传资料、排除已成交和已流失记录的活跃机会，以及尚未完成的活跃任务。**企业展示**、企业档案、**产品中心**、资料、机会、任务和 **Assistant** 保留为独立视图。空状态引导用户进入相应设置或创建操作。概览不展示示例业务结果，也不根据记录数量推断营收和增长。
...
点击**读取资料并建档**，向持久化建档会话发送明确请求。Agent 遍历完整清单，分页读取支持的文档，保存有来源的企业与产品草稿，再询问缺失或冲突的信息。`enterprise_sources` 记录已返回的提取片段，只有全部片段已读取，文件才能标记为已使用。空文档、解析失败、截断或不支持的内容需要补充资料或明确排除理由。图片、视频和 ZIP 保留为素材；不提供 OCR、视觉理解和压缩包展开。文件夹内容只作为参考资料，不作为指令。
