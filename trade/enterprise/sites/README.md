...
Public pages use `/sites-live/<site-id>/` on the current port. HTML, styles, scripts and embedded assets come from the prepared build; source receipts and private inbox records are not served. Pages use a sandboxed opaque origin without access to the editor's DOM or cookies. Scripts can submit only to that site's inquiry route under the response policy; native form navigation is disabled. Static page compilation does not run generated scripts on the Host. Local publication supports HTML pages with bundled local dependencies; server-side Next.js execution, arbitrary asset downloads and dynamic server routes require another provider.
...
The inbox displays the newest 100 inquiries and the total count. Authorized workspace users can mark records read, close them or permanently delete them. Receipt does not send email or synchronize a CRM. Operators own response handling, access and deletion requests, and retention policy.
...
For a company server, install and build the same repository and enterprise plugin, retain its `templates/` directory, create the `trade` profile from the shipped `web` profile on first launch, and run the supported `dsh` CLI with the [deployment patch](../../cordis.patch.yml), a durable `DSH_HOME`, and a loopback listener. Put an HTTPS reverse proxy in front of **only** `/sites-live/`, forwarding the path unchanged. Keep `/api/`, authentication links and the workspace UI off that public route; access administration through the existing authenticated private connection. Set enterprise `publicBaseUrl` to the external origin so inquiry CSP matches the visitor URL. Do not expose the entire local development workspace as a multi-tenant public service.
