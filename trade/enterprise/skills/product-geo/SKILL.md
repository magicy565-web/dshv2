...
Use action `assess` for each imported file: `used` requires all extracted passages to have been read; `excluded` requires a concrete reason why the source is outside the requested scope; `needs_input` identifies missing text, conflicting information, unsupported media or a required unreadable source. Images, videos and ZIP archives are stored, not visually interpreted or recursively unpacked. Scanned documents require a text version because OCR is unavailable. Do not label such material `used`. Ask for missing content or the user's decision to exclude it. Inspect the inventory again before final scope confirmation; unresolved and incomplete uploads cannot silently count as covered.
