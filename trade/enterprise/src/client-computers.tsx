...
export function ComputersPanel({ t }: PropsLocale<'computers'>) {
  ...
  return <section className="ent ent-computers"><div className="ent-inner">
    ...
    {visibleBindings?.map(item => <section className="ent-section cm-binding" key={item.id}>
      ...
      <p>{t(data?.routines.includes(item.id) ? 'routineConfigured' : 'routineMissing')}</p>
      <p><a href={t('routineGuideUrl')} target="_blank" rel="noreferrer">{t('routineGuide')}</a></p>
      <h3>{t('jobs')}</h3>
      ...
      {data?.jobs.filter(job => job.computerId === item.id).slice().reverse().map(job => <article className="ent-task-row" key={job.id}>
        <div><h3>{job.objective}</h3><span className="cm-job-state" data-state={job.state}>{t(job.state)}</span><p className="ent-task-description">{job.progress}</p><small>{t('activity')}: {job.lastProgressAt ? new Date(job.lastProgressAt).toLocaleString() : t('noActivity')}</small>
          {data.wakes.filter(wake => wake.jobId === job.id).map(wake => <p key={wake.jobId}>{t(wake.state === 'accepted' ? 'wakeAccepted' : wake.state === 'rejected' ? 'wakeRejected' : wake.state === 'sending' ? 'wakeSending' : 'wakeUnknown')}{wake.status !== null && ` (HTTP ${wake.status})`}</p>)}
          ...
          {item.enabled && data.routines.includes(item.id) && ['QUEUED', 'WAITING_HUMAN', 'WAITING_APPROVAL', 'CANCEL_REQUESTED'].includes(job.state) && <Button disabled={busy || data.wakes.some(wake => wake.jobId === job.id && (wake.state === 'sending' || wake.state === 'unknown' || job.state === 'QUEUED' && wake.revision === job.revision && wake.state === 'accepted'))} onClick={() => { void request({ action: 'wake', id: job.id, expectedRevision: job.revision }) }}>{t('wakeRoutine')}</Button>}
