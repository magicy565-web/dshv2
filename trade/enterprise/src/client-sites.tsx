...
export function SitesPanel({ t, generate }: Props) {
  ...
  const useStarter = () => action(async () => {
    ...
    setTab('preview'); setPrompt(''); setRefresh(value => value + 1)
  })
  ...
  return <section className="ent site-workspace"><div className="site-shell">
    ...
    <main className="site-main">{!site ? <div className="site-welcome"><SiteExperience t={t} />{promptForm}<h2 className="site-creation-heading">{t('creationOptions')}</h2><div className="site-creation-options"><SiteCompanyPanel t={t} style={templateStyle} created={result => { setSites(items => [...items, result]); setSelected(result.id); setRevision(result.currentRevisionId); setTab('preview'); setPrompt(''); setRefresh(value => value + 1) }} /><article className="site-starter"><h3>{t('starterTitle')}</h3><p>{t('starterDetail')}</p><div className="site-template-options"><label>{t('templateBrand')}<Input disabled={busy} maxLength={80} value={brandName} placeholder={t('templateBrandHint')} onChange={event => setBrandName(event.target.value)} /></label><label>{t('templateStyle')}<select disabled={busy} value={templateStyle} onChange={event => setTemplateStyle(event.target.value)}>{(['industrial', 'precision', 'international'] as const).map(style => <option key={style} value={style}>{t(style)}</option>)}</select></label></div><Button disabled={busy || loading || !template} onClick={() => { void useStarter() }}>{t('useStarter')}</Button></article></div></div> : <>
      ...
      {promptForm}
      <SiteLocalPanel key={`${site.id}:${selectedRevision}`} siteId={site.id} revisionId={selectedRevision} t={t} publicationChanged={() => setRefresh(value => value + 1)} />
