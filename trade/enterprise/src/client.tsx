...
function Panel({ t, useEnterprise, load, save, upload, rename, remove, task, opportunity, generate }: Props) {
  const state = useEnterprise(value => value)
  const [tab, setTab] = useState<'profile' | 'assets' | 'ai' | 'tasks' | 'opportunities' | 'overview' | 'identity' | 'offerings' | 'fit' | 'capabilities' | 'constraints' | 'evidence'>('overview')
