...
export function siteTools(sites: SiteService, tenantId: TenantId, maxBytes: number, hosting?: SiteHosting): readonly ToolDefinition[] {
  ...
  const define = <T>(name: string, description: string, schema: z.ZodType<T>, execute: (input: T) => unknown | Promise<unknown>): ToolDefinition => ({
    ...
    async execute(args, execution) {
      ...
      return execute(schema.parse(args))
    },
  })
  return [
    ...
    define('site_preview', 'Build a saved static website revision and return its private preview URL and file manifest. This does not publish the website. Next.js requires an isolated framework build provider.',
      z.object(revision).strict(), async input => {
        const build = sites.build(spec(input.siteId), SiteRevisionId(input.revisionId))
